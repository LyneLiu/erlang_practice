...
-export([help/0,lc/1,c/1,cplus/1,c/2,nc/1,nc/2, nl/1,l/1,i/0,i/1,ni/0,
         y/1, y/2,
...
help() ->
    io:put_chars(<<"bt(Pid)    -- stack backtrace for a process\n"
		   ...
		   "cplus(File)-- compile and load code in <File>\n"
...
-spec cplus(File) -> {'ok', Module} | 'error' when
      File :: file:name(),
      Module :: module().

cplus(File) -> c(File, debug_info).
