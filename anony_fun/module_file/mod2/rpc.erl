...
fun_call(Node,ModList)  ->
    [fun_call_1(Node,Mod) || Mod <- ModList].

fun_call_1(Node,Mod)  ->
    case rpc:call(Node,erlang,module_loaded,[Mod]) of
        true  ->
	    %% 模块在远程节点已加载，判断节点间模块版本是否一致
	    AttrList1 = Mod:module_info(attributes),
	    {vsn,Vsn1} = lists:keyfind(vsn,1,AttrList1),
            AttrList2 = rpc:call(Node,Mod,module_info,[attributes]),
	    {vsn,Vsn2} = lists:keyfind(vsn,1,AttrList2),
            case Vsn1 =:= Vsn2 of
                  true  ->
                        ok;
                  false ->
...
%% 模块导出函数的rpc处理:同步依赖模块列表
exp_call(Node,ModList)  ->
    [exp_call_1(Node,Mod) || Mod <- ModList].


exp_call_1(Node,Mod)  ->
    case rpc:call(Node,erlang,module_loaded,[Mod]) of
        true    ->
	    %% 模块加载的情况下，判断对方节点是否已经保存模块beam文件
            FileName = "./" ++ atom_to_list(Mod) ++ ".beam",
            case rpc:call(Node,file,read_file_info,[FileName])  of
                {ok,_FileInfo}   ->
                    {ok,{_,[Vsn1]}} = beam_lib:version(Mod),
                    {ok,{_,[Vsn2]}} = rpc:call(Node,beam_lib,version,[Mod]),
                    case Vsn1 =:= Vsn2 of
                        true    ->
                            ok;
                        false   ->
        ...
        false   ->
            exp_load(Node,Mod);
       Other	->
	          io:format("error info:~p.~n",[Other]),
            Other
    end.

%% 将模块加载至对方节点，并保存模块beam文件
exp_load(Node,Mod)  ->
    {Mod,Bin,File} = code:get_object_code(Mod),
    case rpc:call(Node,code,load_binary,[Mod,File,Bin]) of
        {badrpc,Reason1}  ->
